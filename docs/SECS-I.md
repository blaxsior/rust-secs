...
1. send
    ...
    - reply linking
        ...
        - R-bit가 설정된 경우
        - 동일 device ID + 동일 system bytes + secondary message의 첫번째 블록 + R 비트 반전 
    - reply timeout(t3)
...
2. receive: 블록들을 식별 및 적절한 메시지로 조립
    ...
    - inter block timeout(t4)
        - multi block message에서 block 수신 성공 간 시간 간격
        - 시간 초과 시 메시지 전송 취소 / 트랜잭션 중단
        - 각각의 multi block message마다 개별 timer 가져야 함.
        - 연속된 블록이 들어오면 초기화
3. algorithm: header 비교해서 expected block인지 체크
    - if not expected block
        - primary -> ok
        - else -> 블록 에러 + 버림
    - E-bit == 0 -> inter block timer(t4) 설정 + block number += 1
